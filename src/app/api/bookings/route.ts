...
export async function GET(request: NextRequest) {
  try {
    ...
    if (session.user.role === 'customer') {
      bookings = bookings.filter(b => b.CustomerID === parseInt(session.user.id || '1001'));
    } else if (session.user.role === 'agent') {
      bookings = bookings.filter(b => b.AgentID === parseInt(session.user.id || '101'));
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newBooking = {
      ...
      AgentID: session.user.role === 'agent' ? parseInt(session.user.id || '101') : null,
